 ...
 * packaging to "maven-archetype" if the inspected JAR is an Archetype. Since
 * packaging is already handled by Minimal creator, this Creator only alters the
 * supplied ArtifactInfo packaging field during processing, but does not
 * interferes with Lucene document fill-up or the ArtifactInfo fill-up (the
 * update* methods are empty).
 *
 * @author cstamas
 */
